...
async function fetchData(event) {
    ...
    const response = await fetch('/ApiLender/Borrower?status=' + status, {
        method: 'POST',
        headers: {
        ...
        data.forEach(item => {
            ...
            row.innerHTML = `
                <tr class="hover">
                    ...
                    <td>
                        <button class="btn btn-outline btn-primary btn-sm ${item.status != `Requested` && `btn-disabled`}">
                            Fund
                        </button>
                    </td>
                </tr>
            ...
            row.querySelector('button').addEventListener('click', () => openFundingModal(item));
        });
    }
    else {
        const row = document.createElement('tr');
